...
// instead of being reinterpreted as garbage. '3' = text and credential in RAM.
static const uint8_t EE_MAGIC[4] = { 'F', 'L', 'B', '3' };

// ---------------------------------------------------------------------------
// Volatile state. Cleared by the C runtime at every reset, which is the whole
// point: power loss erases the secret.
// ---------------------------------------------------------------------------
static uint8_t  textBuf[TYPEOUT_MAX];
static uint16_t textLen;
...
{
  return rk.valid && memcmp(rk.rpIdHash, rpIdHash, 32) == 0;
}

bool store_rk_nonce(uint8_t nonce[CRED_NONCE_LEN])
{
  if (!rk.valid) return false;
  memcpy(nonce, rk.nonce, CRED_NONCE_LEN);
  return true;
}

uint8_t store_rk_userid(uint8_t userId[MAX_USER_ID_LEN])
{
  if (!rk.valid) return 0;
  memcpy(userId, rk.userId, rk.userIdLen);
  return rk.userIdLen;
}
                  ...
                  const uint8_t *userId, uint8_t userIdLen)
{
  ...
  memcpy(rk.rpIdHash, rpIdHash, 32);
  memcpy(rk.nonce, nonce, CRED_NONCE_LEN);
  memcpy(rk.userId, userId, userIdLen);
  rk.userIdLen = userIdLen;
  rk.valid = true;
}

// --- type-out text -----------------------------------------------------------

uint16_t store_text_len(void) { return textLen; }

uint8_t store_text_byte(uint16_t i)
{
  return i < textLen ? textBuf[i] : 0;
}
...
{
  if (len < 1) return;
  text_store(src + 1, len - 1, src[0] ? textLen : 0);
}
